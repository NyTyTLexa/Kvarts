...
using ContractsMessaging = ProcurementSystem.Contracts.Messaging;

namespace ProcurementSystem.Services.Logistics.BackgroundServices;

/// <summary>
/// Раз в секунду выбирает необработанные записи Outbox, публикует их в NATS JetStream
/// и проставляет ProcessedAtUtc. Так гарантируется доставка событий даже при кратковременной
    ...
    ILogger<OutboxProcessor> logger) : BackgroundService
{
        ...
        for (int attempt = 1; !ct.IsCancellationRequested; attempt++)
        {
            try
            {
                await js.CreateOrUpdateStreamAsync(
                    new StreamConfig(ContractsMessaging.Stream, [ContractsMessaging.SubjectWildcard]), ct);
                logger.LogInformation("JetStream-стрим '{Stream}' готов", ContractsMessaging.Stream);
                return;
            }
    ...
    private async Task ProcessBatchAsync(CancellationToken ct)
    {
        ...
        {
            await bus.PublishAsync(ContractsMessaging.Subject(m.Type), Encoding.UTF8.GetBytes(m.Payload), ct);
