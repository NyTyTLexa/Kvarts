...
internal sealed class RelevanceMatcher(ICatalogClient catalog, MatchingModelCache cache) : IRelevanceMatcher
{
    public int TrainedOn => cache.TrainedOn;

    public async Task EnsureReadyAsync(CancellationToken ct = default)
    {
        var count = await catalog.CountProductsAsync(ct);
        if (count == cache.TrainedOn && cache.Rows.Count > 0) return;
        await cache.Gate.WaitAsync(ct);
        try
        {
            count = await catalog.CountProductsAsync(ct);
            if (count == cache.TrainedOn && cache.Rows.Count > 0) return;
    ...
    private IEnumerable<Guid> CollectCandidates(string? sku, string name)
    {
    ...
    {
        var products = (await catalog.ListAllProductsAsync(ct))
