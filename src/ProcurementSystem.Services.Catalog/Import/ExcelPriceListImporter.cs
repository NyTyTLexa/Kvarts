...
namespace ProcurementSystem.Services.Catalog.Import;
...
/// Лист с таблицей цен — первый, на котором DetectColumns нашёл заголовки (не обязательно
/// Worksheets.First). Иерархия собирается с остальных листов. Старый .xls копирует все листы.
/// </summary>
public class ExcelPriceListImporter(CatalogDbContext db) : IPriceListImporter
{
    ...
    public async Task<ImportResult> ImportAsync(Guid vendorId, Stream excel, string fileName, string? uploadedBy, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                // Лист с таблицей цен пропускаем: CollectHierarchy без «уровень иерархии» no-op,
                // но на ценовом листе те же колонки «Артикул» не должны затирать путь.
    ...
    /// ClosedXML напрямую; старый бинарный .xls (OLE2-сигнатура) — через NPOI, значения всех листов
    ...
    private static XLWorkbook LoadWorkbook(byte[] bytes)
    {
        ...
        if (!isOle2)
        {
            ...
            {
                // ClosedXML разбирает и графику тоже, поэтому падает на книгах, где картинки
                // лежат без имени: «Picture names cannot be empty». А фото товаров прямо
                // в прайсе — обычная практика: у VALTEC их 1269, у STOUT больше двухсот.
                // NPOI рисунки не читает вовсе, поэтому такие книги открывает спокойно.
                // Если и он не справился — значит файл действительно битый, и наружу
                // уходит исходная ошибка, а не подменённая.
                try
                {
                    using var xlsx = new MemoryStream(bytes);
                    using var modern = new NPOI.XSSF.UserModel.XSSFWorkbook(xlsx);
                    return CopyToClosedXml(modern);
                }
                catch
                {
                    throw primary;
        ...
        return CopyToClosedXml(legacy);
    }

    /// <summary>
    /// Переливает значения всех листов книги NPOI в in-memory книгу ClosedXML,
    /// чтобы дальше работал один общий конвейер разбора — независимо от того,
    /// каким читателем файл удалось открыть.
    /// </summary>
    private static XLWorkbook CopyToClosedXml(NPOI.SS.UserModel.IWorkbook source)
    {
        var wb = new XLWorkbook();
        var usedNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        for (var i = 0; i < source.NumberOfSheets; i++)
        {
            var sheet = source.GetSheetAt(i);
            if (sheet is null) continue;
            var ws = wb.AddWorksheet(UniqueSheetName(sheet.SheetName, usedNames));
            CopySheet(sheet, ws);
        }
    ...
    private static void CopySheet(NPOI.SS.UserModel.ISheet sheet, IXLWorksheet ws)
    {
    ...
    /// <summary>ClosedXML: имя листа ≤ 31 символ, без : \ / ? * [ ], уникально в книге.</summary>
    private static string UniqueSheetName(string? raw, HashSet<string> used)
    {
        ...
        while (!used.Add(candidate))
        {
            ...
            candidate = (baseName.Length <= keep ? baseName : baseName[..keep]) + suffix;
        }
        return candidate;
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(raw)) return "Лист1";
        var chars = raw.Trim().ToCharArray();
        for (var i = 0; i < chars.Length; i++)
        {
            if (chars[i] is ':' or '\\' or '/' or '?' or '*' or '[' or ']')
                chars[i] = '_';
        }
        var name = new string(chars).Trim();
        if (name.Length == 0) return "Лист1";
