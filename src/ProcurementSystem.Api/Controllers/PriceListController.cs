...
public class PriceListController(IOfferService offers, IPriceListImporter importer, IPriceHistoryService history, IPriceListCorpus corpus) : ControllerBase
{
    ...
    /// <summary>
    /// Стенд ML: 100+ пересекающихся прайсов и каталог на тысячи SKU + кривая заявка.
...
/// Swashbuckle не умеет разбирать метод, где [FromForm] стоит на простом параметре рядом
/// с IFormFile: генерация падает, и вместе с ней рушится ВЕСЬ документ OpenAPI —
/// Scalar показывает пустую страницу. Собранные в класс поля он разбирает нормально.
/// Привязка не меняется: поле формы vendorId ложится в VendorId, file — в File.
/// </summary>
