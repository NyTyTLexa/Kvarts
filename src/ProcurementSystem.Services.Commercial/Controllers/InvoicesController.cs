...
namespace ProcurementSystem.Services.Commercial.Controllers;

/// <summary>
/// Счета NOC: создание из согласованного КП и продвижение по стадиям ЖЦ (ТЗ п.B7).
/// Стадии «Пришёл на склад» и «Отражено в 1С» выставляет склад через POST .../status.
/// </summary>
[ApiController]
[Route("api/invoices")]
...
public class InvoicesController(IInvoiceQueryService queries, IInvoiceCommandService invoices) : ControllerBase
{
    ...
    /// <summary>Продвинуть счёт по стадиям ЖЦ (ТЗ 10.11 / UC-08). Политика setInvoiceStatus;
    /// какую стадию можно ставить — <see cref="Invoice.ActorMaySet"/> (403), линейность — TryTransitionTo (409).</summary>
