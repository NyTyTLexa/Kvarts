...
public class InvoiceQueryService(CommercialDbContext db) : IInvoiceQueryService
{
    ...
    public async Task<IReadOnlyList<InvoiceAttachmentDto>> ListAttachmentsAsync(Guid invoiceId, CancellationToken ct = default) =>
        await db.InvoiceAttachments.AsNoTracking()
    ...
    {
        var a = await db.InvoiceAttachments.AsNoTracking()
